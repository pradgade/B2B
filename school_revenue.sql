...
-- The schools with auto renewal are called automated auto renew in the note, making a combined table for them.
	...
	WHERE note LIKE '%Automated auto renew%';
...
CALL p_credit_note_link();

-- Event code

CREATE EVENT IF NOT EXISTS analytics.e_credit_note_link
	ON SCHEDULE
		EVERY 1 DAY
	...
	BEGIN
		--
		CALL `analytics_procedure_logging_start`('e_credit_note_link');
		--
		CALL `p_credit_note_link`();
		--
		CALL `analytics_procedure_logging_stop`('e_credit_note_link');
		--
	END;